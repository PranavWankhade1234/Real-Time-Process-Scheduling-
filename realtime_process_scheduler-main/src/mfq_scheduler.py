...
class MFQScheduler:
    ...
    def schedule(self, processes):
        ...
        for process in processes:
            # Initially place processes in the highest priority queue
            ...
            for i in range(self.num_queues):
                if self.queues[i]:
                    ...
                    self.current_time += min(
                        self.time_quantum[i], process.burst_time)
                    process.burst_time -= min(
                        self.time_quantum[i], process.burst_time)
                    if process.burst_time > 0:
                        self.queues[min(i + 1, self.num_queues - 1)
                                    ].append(process)
                    break
